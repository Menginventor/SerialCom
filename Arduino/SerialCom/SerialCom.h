...
class SerialCom
{
  public: // <<---------------------------------------//public
	...
	uint8_t sendData(const uint8_t packetID = 0);
	uint8_t available();
	uint8_t currentPacketID();


	/*
	 uint16_t SerialCom::txObj(const T &val, const uint16_t &index=0, const uint16_t &len=sizeof(T))
	 Description:
	 ------------
	...
	*/
	...
	template <typename T>
	uint16_t addData(const T& val, const uint16_t& len = sizeof(T))
	{
		uint16_t index = crr_addData_idx;
		crr_addData_idx += len;
		return packet.txObj(val, index, len);
	...
	/*
	 uint16_t SerialCom::rxObj(const T &val, const uint16_t &index=0, const uint16_t &len=sizeof(T))
	 Description:
	 ------------
	...
	*/
	...
	template <typename T>
	uint16_t readData(const T& val, const uint16_t& len = sizeof(T))
	{
		uint16_t index = crr_readData_idx;
		crr_readData_idx += len;
		return packet.rxObj(val, index, len);
	}


	/*
	 uint8_t SerialCom::sendDatum(const T &val, const uint16_t &len=sizeof(T))
	 Description:
	 ------------
	  * Stuffs "len" number of bytes of an arbitrary object (byte, int,
	  float, double, struct, etc...) into the transmit buffer (txBuff)
  ...
  private: // <<---------------------------------------//private
	Stream* port;
	uint16_t crr_addData_idx = 0;
	uint16_t crr_readData_idx = 0;
};
